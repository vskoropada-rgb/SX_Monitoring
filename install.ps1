...
#
# Запуск одним рядком у PowerShell (від Адміністратора):
...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)) {
    ...
    Write-Host ""
    Write-Host "  Клікніть правою кнопкою на PowerShell → 'Запуск від імені адміністратора'" -ForegroundColor Yellow
    Write-Host ""
...
if (-not (Test-Path "D:\")) {
    ...
    Write-Host ""
    Write-Host "  Альтернативи:" -ForegroundColor White
    Write-Host "  [1] C:\setup\monitoring-sc" -ForegroundColor White
    Write-Host "  [2] Ввести свій шлях"        -ForegroundColor White
    Write-Host ""
    $ans = Read-Host "  Вибір (1/2)"
    if ($ans -eq "2") {
        $INSTALL_DIR = Read-Host "  Шлях встановлення"
        if (-not $INSTALL_DIR) { Write-Err "Шлях не вказано."; exit 1 }
    } else {
        $INSTALL_DIR = "C:\setup\monitoring-sc"
    }
}
...
if ($countFail -eq 0) {
    ...
    Write-Ok "Завантажено: $countOk"
    Write-Info "Пропущено:   $countFail  (файли, яких ще немає в репо)"
}
...
if (-not (Test-Path $managePath)) {
    Write-Err "manage.ps1 не знайдено після завантаження — перевірте підключення."
    Read-Host "  Натисніть Enter"
    exit 1
}
...
# Запускаємо manage.ps1 у новому процесі PowerShell щоб $MyInvocation.MyCommand.Path
# розпізнав правильний $ScriptDir
